<?php
/**
 * SAML 2.0 remote SP metadata for SimpleSAMLphp.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-sp-remote
 */

// Custom host with port number (for testing from browser on host O/S):
$metadata['http://mfa-sp.local:52021'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://mfa-sp.local:52021/module.php/saml/sp/saml2-acs.php/mfa-idp',
	'SingleLogoutService' => 'http://mfa-sp.local:52021/module.php/saml/sp/saml2-logout.php/mfa-idp',
    'certData' => '',
];

// Container name, no port number (for testing from another docker container):
$metadata['http://mfasp'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://mfasp/module.php/saml/sp/saml2-acs.php/mfa-idp-no-port',
	'SingleLogoutService' => 'http://mfasp/module.php/saml/sp/saml2-logout.php/mfa-idp-no-port',
    'certData' => '',
];

// Copy both (modified) for use by local password manager SP.
$metadata['http://mfa-pw-manager.local:52022'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://mfa-pw-manager.local:52022/module.php/saml/sp/saml2-acs.php/mfa-idp',
	'SingleLogoutService' => 'http://mfa-pw-manager.local:52022/module.php/saml/sp/saml2-logout.php/mfa-idp',
    'certData' => '',
];
$metadata['http://mfapwmanager'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://mfapwmanager/module.php/saml/sp/saml2-acs.php/mfa-idp-no-port',
	'SingleLogoutService' => 'http://mfapwmanager/module.php/saml/sp/saml2-logout.php/mfa-idp-no-port',
    'certData' => '',
];
