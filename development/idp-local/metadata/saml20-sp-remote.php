<?php
/**
 * SAML 2.0 remote SP metadata for SimpleSAMLphp.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-sp-remote
 */

use Sil\PhpEnv\Env;

$spHostAndPort = 'mfasp' . Env::get('TEST_SP_PORT');

/*
 * Example SimpleSAMLphp SAML 2.0 SP
 */
$metadata['http://' . $spHostAndPort] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://' . $spHostAndPort . '/module.php/saml/sp/saml2-acs.php/mfa-idp',
	'SingleLogoutService' => 'http://' . $spHostAndPort . '/module.php/saml/sp/saml2-logout.php/mfa-idp',
    'certData' => '',
];
